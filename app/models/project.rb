...
class Project < ActiveRecord::Base
  ...
  delegate :display_status, :display_progress, :display_image, :display_expires_at,
    :display_pledged, :display_goal, :remaining_days, :display_video_embed_url, :progress_bar, :successful_flag, :currency_symbol, :currency_delimiter,
    to: :decorator
  ...
  scope :near_of, ->(country) { joins(:user).where("lower(users.country) = lower(?)", country) }
  ...
  validates_presence_of :name, :user, :category, :about, :headline, :goal, :permalink, :uploaded_image
  ...
  def donators_total
    project_total ? project_total.donators_total: 0
  end
